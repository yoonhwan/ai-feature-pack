...
planner_rounds: 1   # 기획 라운드 — planner 디스패치 시점에 +1 (한도 2)
...
2. **게이트/검증 디스패치·판정 수신 시** — 카운터(da_round/planner_rounds)는 **디스패치 시점에 +1 기록**, 판정(DA APPROVED·CHANGES_REQUESTED, tester ALL_PASS·FAIL) 원문은 `state/<slug>/` 산하 파일로, state.md에는 결과 한 줄.
...
**항상 디스크에 있어야 하는 상태의 완결 목록**: **ACTIVE 포인터**, 피처 파일, **파이프라인 형상(pipeline/da)**, 설계 파일(버전별), stage 포인터, 3종 카운터(da_round/planner_rounds/respawns), **브레인 세션 id(brain_sessions)**, 원장, 워커 산출물(판정·증거·구현 보고), 블록 사유. 이 목록 밖의 것(워커 transcript, Workflow runId)은 복원에 **불필요**해야 한다 — runId는 이벤트 로그에 참고로 기록하되 복원 시 신뢰하지 않는다(같은 세션 한정이므로).
...
**워커 자기 윈도우 정책**: 워커 자신의 ctx %도 오케스트레이터가 폴링할 수 없다 — 장수명 워커(implementer, da 드라이버) 계약(템플릿)에 self-checkpoint를 넣는다: 자기 윈도우 압박을 자각하면 진행분을 파일로 flush하고 team-lead에 `WINDOW_PRESSURE` 1줄 보고 후 지시 대기 → 오케스트레이터가 flush 확인 후 해산·재스폰(계획적 재스폰 — respawns 한도 비소모). 일회성 워커(checker/planner/tester)는 누적이 없어 해당 없음.
...
**approve loop 라운드 간 상태 보존**: 라운드 상태는 전부 파일에 있다 — `da_round` 카운터(state.md) + `state/<slug>/da-round<N>.md`(판정·증거) + `features/design-<slug>-v<N>.md`(재기획 시 planner가 **v+1 새 파일**로 Write, frontmatter `design:` 갱신 — v1/v2가 파일 존재로 구분). implementer가 살아있으면 SendMessage "설계 파일이 갱신됨(v2 경로 전달). 재Read 후 차이만 구현" — 빠른 경로. 죽었으면 재스폰 — 같은 계약이므로 결과 동일.
...
4. **산출물 실재 검증** (state는 선언, 파일이 증거): stage 포인터 이하 완료 단계 중 **형상에 포함된 단계만** 산출물 존재를 확인 — 피처 파일(stage 0), checker JSON(1), `design-<slug>-v<planner_rounds>.md`(2), `impl-round<planner_rounds>.md`(3 — 현재 설계 버전 대응만 유효, 이전 라운드는 stale), tester/da 라운드 파일(4-5). **불일치 시 산출물이 실재하는 마지막 단계로 stage 롤백** (예: state=3인데 설계 파일 없음 → stage 2부터).
...
4. **축약 형상 복원**: 파이프라인 축약(확인→구현→테스트) + `da: none` 피처가 stage 3 진행 중 사망 → 새 세션 복원 → 기대: 형상 게이팅에 따라 planner 스폰·da-round 대기 **없이** checker JSON 확인 → 구현 재실행(구현 SSOT = 피처 파일)으로 직행.
