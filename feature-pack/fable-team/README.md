...
| planner | fable5 / effort **high** (max 금지 — hang 실측) | Workflow | 원인 분석·해결 설계 → 설계 파일 산출 |
...
핵심 불변식: ① 워커 `tools:`에서 Agent/Task 제외 → **서브의 서브 스폰 차단** ② planner 외 워커에 fable-5 금지 ③ 단계 간 전달은 파일 경유(설계 파일) → 오케스트레이터 컨텍스트 최소화 ④ 라운드·복원 판정은 카운터 산술이 아니라 **파일 실재 기준**.
...
2. **워커별 브레인/effort 질문** — 기본값 그대로 엔터만 쳐도 표준 로스터가 구성된다. planner 외 워커에 fable-5는 거부된다.
...
mul 음수 버그 수정 사이클: planner(fable5 — 당시 max, 현 표준 high) 설계 파일 → implementer(opus4.6) 2줄 패치 → tester ALL_PASS(8케이스) → DA(codex gpt-5.5/xhigh) APPROVED — 오케스트레이터 개입 없이 파일 릴레이만으로 완주. 컨텍스트 관리 설계는 DA(codex) 3라운드 + critic(opus) 5라운드 + planner(fable) 교차 게이트 전건 APPROVED.
