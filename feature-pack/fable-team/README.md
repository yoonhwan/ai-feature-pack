...
- **"FT 하자"** (또는 "FT 구성", "fable-team") 로 트리거 → 에이전트가 먼저 `.fable-team/state/ACTIVE`를 확인해 **진행 중이던 파이프라인이 있으면 복원**([context-management.md](skill/references/context-management.md) §4), 없으면 [피처 인터뷰](skill/references/feature-interview.md)부터 시작한다.
- 이후 6단계 파이프라인(수집→기획→구현→검증→게이트→종결)이 파일 릴레이로 돈다. 오케스트레이터 개입 최소.
