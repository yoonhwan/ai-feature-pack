...
![fable-team](docs/images/readme-banner.jpg)
...
|----|--------------------------|-----------|------|
| 오케스트레이터 | sonnet-5 또는 fable-5 (ultracode — 세션 시작 시 사용자 선택) | 현 세션 | 태스크 분해, 전달, 모니터링 루프, 게이트 분기 — **기획·문제해결 금지** |
| architect | fable5 / effort **high** (max 금지 — hang 실측) | Workflow | 원인 분석·해결 설계 → 설계 파일 산출 |
| checker × N | sonnet4.6 / medium | Agent | 문서·코드·로그 확인 (병렬) |
| implementer | opus4.6 / high | Agent | 설계 파일 기반 구현 + 프로젝트 Skill 호출 |
...
핵심 불변식: ① 워커 `tools:`에서 Agent/Task 제외 → **서브의 서브 스폰 차단** ② architect 외 워커에 fable-5 금지 ③ 단계 간 전달은 파일 경유(설계 파일) → 오케스트레이터 컨텍스트 최소화 ④ 라운드·복원 판정은 카운터 산술이 아니라 **파일 실재 기준**.

위 표를 실제 통신 구조로 그리면 아래와 같다 — 오케스트레이터가 유일한 허브이고, 각 워커와의 통신은 동기 호출이 아니라 `.signals/` 파일 센티널을 통한 비동기 릴레이다. 구현↔검증DA는 승인까지 반복되는 루프, 메모리(PM)는 오케 지시 없이도 상시 독립적으로 도는 별도 프로세스, watchd는 전 세션을 가로질러 hang만 감시하는 크로스커팅 레이어다.

![fable-team 허브-스포크 구조](docs/images/hub-spoke-diagram.png)
...
2. **워커별 브레인/effort 질문** — 기본값 그대로 엔터만 쳐도 표준 로스터가 구성된다. architect 외 워커에 fable-5는 거부된다.
...
mul 음수 버그 수정 사이클: architect(fable5 — 당시 max, 현 표준 high) 설계 파일 → implementer(opus4.6) 2줄 패치 → tester ALL_PASS(8케이스) → DA(codex gpt-5.5/xhigh) APPROVED — 오케스트레이터 개입 없이 파일 릴레이만으로 완주. 컨텍스트 관리 설계는 DA(codex) 3라운드 + critic(opus) 5라운드 + architect(fable) 교차 게이트 전건 APPROVED.
