---
name: fable-team
description: 일반화된 팀 오케스트레이션 하네스. "FT 구성", "FT 해보자", "FT 하자" 요청 시 사용 (보조 트리거: "fable-team", "팀 구성", "팀 에이전트 설치", "팀으로 진행" / 로컬 패치는 "FT 업데이트"). 오케스트레이터(사다리 최상위 가용 모델)는 전달·조율만, 기획·문제해결은 planner 브레인(기본 fable5 high — D1 표준)이 전담. 설치 인터뷰 + 피처 설계 인터뷰로 프로젝트별 워커를 커스텀 생성한다. 트리거 시 부팅 시퀀스(피처 입력→추천 설계→프리뷰→컨펌)가 강제된다.
---
...
| 층 | 담당 | 모델 | 하는 일 | 안 하는 일 |
|----|------|------|---------|-----------|
| **오케스트레이터** | 현재 세션 | 사다리 첫 가용 모델 + effort high(기본 fable5/high — brain-availability §2, 하네스 프로파일 §2-4) | 태스크 분해, 워커 스폰/전달, 커뮤니케이션, 파이프라인 진행, 게이트 판단 릴레이 | **기획·문제해결 금지** — 두뇌 작업을 직접 하지 않아 멈추지 않는 문제해결 루프가 가능 |
| **planner (기획 브레인)** | 서브에이전트 | 기본 fable5 + effort **high**(D1) (미가용 시 사다리: opus-4-8 high → 병렬 opus-4-6 high·sonnet-5 high — D2 역할 적합) | 원인 분석, 해결 설계 — 컨텍스트를 파일/텍스트로 받아 **설계 파일로 반환** | 구현/실행/오케스트레이션 |
...
|------|--------------|--------|------|------|
| ft-planner | **fable5** (설치 시 변경 가능 — 사다리: brain-availability §2) | **high**(D1) | Read, Grep, Glob, Write | 원인 분석·해결 설계 → 설계 파일 |
| ft-checker | sonnet 4.6 | low | Read, Grep, Glob | 문서/코드/로그 확인 (병렬 다수) |
| ft-implementer | opus 4.6 | **high**(D1) | +Bash, Edit, Write, Skill | 설계 파일 기반 구현. 프로젝트 스킬 호출 가능 |
