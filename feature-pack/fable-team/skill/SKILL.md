---
name: fable-team
description: 일반화된 팀 오케스트레이션 하네스. "팀 구성", "fable-team", "팀 에이전트 설치", "팀으로 진행" 요청 시 사용. 오케스트레이터(ultracode 지원 최상위 모델)는 전달·조율만, 기획·문제해결은 planner 브레인(기본 fable5 max)이 전담. 설치 인터뷰 + 피처 설계 인터뷰로 프로젝트별 워커를 커스텀 생성한다.
---
