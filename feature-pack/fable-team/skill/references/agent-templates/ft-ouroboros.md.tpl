...
- **세션 승계(resume 체인)**: session-id를 보고(brain_sessions 기록)하고 후속 진행은 `~/.headroom/claude-hr.sh -p --resume <session-id> --output-format json '<다음 단계>' < /dev/null`로 이어가라. auto/evolve/ralph는 **MCP job-wait을 같은 턴 안에서 자체 폴링**한다 — 네가 별도 폴링 루프를 짤 필요 없고, 끊기면 `--resume`/job_id로 재개.
