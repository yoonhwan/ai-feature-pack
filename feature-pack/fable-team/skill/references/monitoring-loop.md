...
- 오케스트레이터는 중간보고를 받으면 **범위 검사만** 한다 (내용 판단 금지 — 그건 architect 몫):
  ...
  - 설계 자체가 틀렸다는 근거가 오면 → architect에 재기획 라운드 (architect_rounds만 소모, da_round 불변 — 설계 버전 축과 DA 라운드 축은 독립. 오케스트레이터가 판단하지 않는다).
