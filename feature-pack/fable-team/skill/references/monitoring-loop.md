...
파이프라인 중 오케스트레이터는 워커 원장을 유지한다. **원장은 컨텍스트가 아니라 디스크(`.fable-team/state/<slug>.state.md`)가 SSOT** — 자동 컴팩션/세션 재시작/증류에 원장이 증발하지 않도록, 아래 원장을 다음 4개 이벤트마다 **디스크에 write-through**한다: ① 단계 전이 ② 게이트/검증 판정 수신 ③ 워커 상태 변화 ④ 에스컬레이션/블록. 스펙·복원 절차는 `context-management.md` 참조:
