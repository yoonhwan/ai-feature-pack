...
### 핵심 설계 (실측 통과)
- **모델 판별 = transcript_path의 마지막 assistant `message.model`.** 세션별로 정확 — 오케(fable/opus-4-8)만 게이트, 워커(opus-4-6/sonnet)는 자기 세션 모델이 TOP 아니라 자유. 워커가 여러 파일 편집 정상.
...
- env 오버라이드: `OMC_GATE_MAX_CODE_FILES`(기본 2) / `OMC_GATE_TOP_MODELS`(기본 `fable|opus-4-8`) / `OMC_DISTILL_WARN_AT`(300000) / `OMC_DISTILL_BLOCK_AT`(450000).
...
- 메인 오케스트레이터(세션) = **opus-4-8 ultracode** (fable 아님). 게이트 대상.
- planner = **opus-4-8 또는 fable5** (FT 구성 시 선택·기록). 설계 파일만 산출(코드 아님)이라 실무상 게이트 무영향.
- 워커: implementer=opus-4-6/medium(heavy=high), tester=sonnet-5/high, checker(대량서치)=sonnet-4-6/high. 게이트 면제.

## opus-4-8[1m] 서브에이전트 모델 leak 교정

[1m] 세션 Agent 스폰 시 워커가 세션 모델 상속(frontmatter 무시) → 전 워커 opus4.8. 교정: ① `.claude/settings.json` env에 `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL` provider ID 지정(tier alias 해석) ② [1m] 세션은 **일회성 브레인 워커(planner/checker/implementer/tester)만** Workflow 경로(explicit model) 강제 — **장수명 드라이버(DA·crew)는 Agent+wrapper 셔틀로 유지**(외부 CLI가 wrapper 주입 full-id로 실행). 일회성 브레인만 스폰 후 `agent-*.meta.json` model 검증(불일치=hard stop).
