...
**2. 설계 승인 → 플랜 (resume)**
```bash
claude -p --resume <sid> --output-format json '스펙 승인. writing-plans로 구현 계획 작성해줘' < /dev/null
```
Expected: bite-sized 태스크 계획 작성, 실행 방식(subagent-driven-development vs executing-plans) 질문으로 종료.

**3. 플랜 실행 (subagent-driven, resume)**
```bash
claude -p --resume <sid> --output-format json 'subagent-driven-development로 실행해줘' < /dev/null
```
...
**5. 완료 게이트 → 마무리 (resume)**
```bash
claude -p --resume <sid> --output-format json 'verification-before-completion 기준으로 증거 확인 후, finishing-a-development-branch로 마무리해줘' < /dev/null
```
...
claude -p --resume <sid> --output-format json '/superpowers:writing-plans 계획 작성' < /dev/null
claude -p --resume <sid> --output-format json '/superpowers:subagent-driven-development 실행' < /dev/null
claude -p --resume <sid> --output-format json '/superpowers:executing-plans 실행' < /dev/null
...
claude -p --resume <sid> --output-format json '/superpowers:dispatching-parallel-agents <실패목록>' < /dev/null
claude -p --resume <sid> --output-format json '/superpowers:requesting-code-review <BASE>..<HEAD>' < /dev/null
claude -p --resume <sid> --output-format json '/superpowers:verification-before-completion 증거확인' < /dev/null
claude -p --resume <sid> --output-format json '/superpowers:finishing-a-development-branch' < /dev/null

# 결과를 파일로 / stdin 프롬프트
claude -p --resume <sid> --output-format json '...' < /dev/null > result.json
cat prompt.md | claude -p --resume <sid> --output-format json - < /dev/null
