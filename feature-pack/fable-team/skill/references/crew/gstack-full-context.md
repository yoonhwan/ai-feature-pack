...
```bash
claude -p --resume <session-id> --output-format json '/context-save 진행상황을 저장해줘' < /dev/null
```
...
```bash
claude -p --resume abc123 --output-format json \
...
```bash
# QA + fix / report-only
claude -p --model claude-sonnet-4-6 --effort high --output-format json --permission-mode acceptEdits '/qa Test and fix the checkout flow.' < /dev/null
claude -p --model claude-sonnet-4-6 --effort high --output-format json '/qa-only Report bugs on the pricing page.' < /dev/null

# Investigate (read-only)
claude -p --permission-mode plan --output-format json '/investigate Why does CI fail but not local?' < /dev/null

# Review / Ship / Land
claude -p --model claude-sonnet-4-6 --effort high --output-format json '/review Check my current diff.' < /dev/null
claude -p --model claude-sonnet-4-6 --effort high --output-format json --permission-mode acceptEdits '/ship Ship the current branch.' < /dev/null
claude -p --model claude-sonnet-4-6 --effort high --output-format json --permission-mode acceptEdits '/land-and-deploy Merge and verify prod.' < /dev/null

# Design / Security
claude -p --model claude-sonnet-4-6 --effort high --output-format json --permission-mode acceptEdits '/design-review Audit and fix the dashboard.' < /dev/null
claude -p --model claude-sonnet-4-6 --effort high --output-format json '/cso Run a daily security audit.' < /dev/null

# Context save/restore + follow-up injection
claude -p --resume <session-id> --output-format json '/context-save Save current progress.' < /dev/null
claude -p --model claude-sonnet-4-6 --effort high --output-format json '/context-restore Resume where I left off.' < /dev/null
claude -p --resume <session-id> --output-format json 'New constraint: do not change the public API.' < /dev/null
