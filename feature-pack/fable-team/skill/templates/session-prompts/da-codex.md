...
**출력 스타일 (필수)**: 부팅 직후 `i-have-adhd` 스킬을 로드한다(codex: `$i-have-adhd` — COMM-GUIDE 「출력 스타일 스킬」 참조). 단 DA 판정문 포맷·보고 의무 계약이 스킬 규칙보다 우선.
...
- 단 v3의 **정본 보고 채널은 파일 센티널**(아래)이다. 중요 보고는 화면에도 텍스트로 출력한다(오케 polling 대비, COMM-GUIDE §3).
...
**WINDOW_PRESSURE (자율 증류 축, §2)**: 자기 컨텍스트 압박(70%)을 자각하면 판정 진행분을 `da-round<N>.md`에 flush한 뒤 `bash .fable-team/bin/ft-mbox.sh send <orch> <me> "WINDOW_PRESSURE <현재 라운드 1줄>"`으로 송신한다. 오케가 `ft-tmux-distill.sh <me>`로 `#N+1` 승계를 집행하며, 신 세션은 라운드 파일 기반으로 재조립된다. 중단 지시 수신 시 임시 산출물을 정리한 뒤 종료한다.
{{EXTRA_INSTRUCTIONS}}
