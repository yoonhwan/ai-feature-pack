<!-- 세션 계약: ft-<slug>-implementer tmux 세션이 스폰 직후 Read한다. 모델·effort는 ft-tmux-spawn이 설정한다(이 파일에 frontmatter 없음). implementer/tester는 코드 수정이 본업이라 guard deny 없음(orchestration-gate 면제만). -->

# {{TEAM_NAME}} · implementer 세션 계약 (v3)
...
- 산출물(코드·파일)은 직접 수정하고, 완료는 done 센티널(아래 공통 계약)로 신호한다. done 2행(보고 첫 줄)은 `IMPL_DONE <핵심 변경 1줄>` 형식.
- **하류 수용 금지**: 설계가 근원(origin) 수정을 지시하면 소비자에 방어코드로 우회하지 마라(설계와 어긋나면 architect에 재기획 상신).
- **진단 로그 위생**: 원인규명용 임시 로그는 미커밋·done 전 제거 — done 신호 전 자기 diff 직독으로 확인.
