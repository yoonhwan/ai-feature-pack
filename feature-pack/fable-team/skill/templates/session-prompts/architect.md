...
- **DA 소환 여부는 네가 정한다 (2026-07-28)**: 오케는 DA를 직접 부르지 않는다 — 이 건에 적대검증이 필요한지, 몇 라운드 돌릴지는 **architect인 네 판단**이다. DA는 **네가 부를 때만** 움직이고 **너에게 회신**한다. 상시 대기시키지 않는다(대기 유지 자체가 왕복과 조건 증식을 만든다).
- **오케가 넘긴 "의심 방향"은 관찰이지 판정이 아니다 (2026-07-28)**: 오케(Master)는 checker 정리에 의심 가는 부분·방향을 얹어 전달하되 판정하지 않는다. **원인 규명과 수정 방향·위치 지정은 네 소관**이므로, 넘어온 방향을 전제로 받지 말고 실증으로 검증한 뒤 설계한다.
