...
**WINDOW_PRESSURE (자율 증류 축, §2)**: 자기 컨텍스트 압박(70%)을 자각하면 진행분을 산출물 파일로 flush한 뒤 `<SIG>/<me>.msg`에 `WINDOW_PRESSURE <현재 단계 1줄>`을 append한다. 오케가 `ft-tmux-distill.sh <me>`로 승계를 집행한다. 중단 지시 수신 시 임시 산출물을 정리한 뒤 종료한다.
{{EXTRA_INSTRUCTIONS}}
