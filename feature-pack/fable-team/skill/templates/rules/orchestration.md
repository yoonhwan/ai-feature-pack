...
**다이렉트 확인 루프 (선택지 — 투 루프 체제)**: "관찰이 곧 판정"인 건(관찰 1~2회 가설 판별·경합가설 계측 1줄 판별·env fresh 선행·빠른 A-B)은 설계 착수 전 즉결 확인 루프로 규명 가속 — 워커 즉결 지시(tester press·impl 행위무변경 계측 1~3줄·checker 읽기전용)로 실증팩(가설+판별결과·실물경로+타임스탬프·재현조건·코드앵커·잔여질문) 완성 후 architect 송부. **fix는 1줄이라도 설계+DA 경유** / 동시 1개(1이슈1스텝) / 실증팩 완성 전 architect·DA 병렬발주 금지. 상세는 fable-team `references/rapid-iteration-loop.md`.
