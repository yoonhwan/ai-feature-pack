...
**안티패턴 (하지 말 것)**: 추측으로 큰 수정 / 여러 이슈 병렬 조사(오케 레벨 빅뱅) / 체커 수집 전 기획 착수 / master가 approve loop 중간 릴레이(판단·재구성 — 단 Legacy 경로의 판단 0 pass-through는 예외) / DA 단순 승인(반박 없이) / 유닛 통과=완성(라이브 관찰 없음 — 완성 증거는 아티팩트에 맞는 형식, JSONL은 예시) / **하류 수용**(근원 버그를 소비자 계약으로 승격 — mapper가 잘못된 타입을 받아들이지 말고 근원에서 고친다; 단 외부·신뢰경계의 소비자 검증은 정당).
