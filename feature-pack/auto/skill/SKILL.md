...
Q4: 실험 브레인 (LLM)?
  (A) 현재 세션 — 지금 모델 그대로 사용 (기본값)
  (B) 모델 지정 — 서브에이전트/팀 워커로 실행할 모델명 입력
      예: sonnet, haiku, opus, codex, gemini-2.5-pro
      → 코딩 에이전트 또는 에이전트 프로그램에서 사용 가능한 모델명 입력
```
...
```
Step 1: 모델 가용성 테스트
  Claude 계열 (opus/sonnet/haiku):
    → Agent(model="<지정모델>", prompt="echo 'brain-check-ok'") 스폰
    → 응답 확인 → OK / FAIL

  외부 CLI (codex/gemini):
    → which codex 또는 which gemini 로 CLI 존재 확인
    → <cli> --version 실행 → OK / FAIL

Step 2: 결과 보고
  성공 → "✓ <모델명> 사용 가능 확인. 이 모델로 자율주행합니다."
  실패 → "✗ <모델명> 사용 불가. 사유: <에러>. 다른 모델을 선택하시거나 현재 세션으로 진행할까요?"
```
...
[AutoResearch 설정 확정]
  ...
  브레인: sonnet (서브에이전트)  ← 현재 세션이면 "현재 세션 (opus)" 표기
```

### 실행 방식 (브레인 설정에 따라)

```
[현재 세션] — 이 세션에서 직접 루프 실행
  → 추가 설정 없음, Workflow 4 진입

[서브에이전트] — Claude 계열 모델 지정 시
  → Agent(model="<모델>", prompt="program.md + 루프 프로토콜") 스폰
  → 메인 세션은 모니터링만

[외부 CLI] — codex/gemini 지정 시
  → 해당 CLI를 Bash로 실행하여 루프 진행
  → 메인 세션이 루프 제어 + 결과 파싱
```
