...
`tts-say` CLI는 macOS 내장 `say`와 ElevenLabs `sag`를 하나로 통합한 TTS 프론트엔드.
에이전트, 바이브코딩, 스크립트에서 `tts-say` 하나만 호출하면 자동으로 최적 엔진을 선택한다.

**할 수 있는 것:**
- 텍스트 → 음성 재생 (`tts-say "텍스트"`)
- 텍스트 → 오디오 파일 저장 (`tts-say speak -o out.mp3 "텍스트"`)
- 엔진별 보이스 목록 조회 (`tts-say voices`)
- 기본 엔진/보이스/옵션 Config 저장 (`tts-say config set ...`)
- stdin 파이프 입력 (`echo "텍스트" | tts-say`)
...
## Step 3: tts-say CLI 설치

이 feature-pack의 `skill/scripts/tts-say` 파일을 PATH에 복사:

```bash
cp skill/scripts/tts-say {{TTS_INSTALL_PATH}}
chmod +x {{TTS_INSTALL_PATH}}
```

> Placeholder `{{TTS_INSTALL_PATH}}`: 설치 경로 (기본: `~/.local/bin/tts-say`)
> 질문: "tts-say CLI를 어디에 설치할까요? (기본: ~/.local/bin/tts-say)"
...
tts-say config set default_engine {{DEFAULT_ENGINE}}

# say 기본 보이스 (한국어)
tts-say config set say.voice Yuna

# sag 기본 설정 (API 키 있는 경우)
tts-say config set sag.voice "{{SAG_DEFAULT_VOICE}}"
tts-say config set sag.model_id eleven_multilingual_v2
tts-say config set sag.lang ko
```

> Placeholder `{{DEFAULT_ENGINE}}`: API 키 여부에 따라 `sag` 또는 `say` (기본: `auto`)
> Placeholder `{{SAG_DEFAULT_VOICE}}`: 사용자 선호 보이스. `tts-say voices --engine sag` 로 목록 확인 후 선택
...
```bash
cp -r skill/ {{OPENCLAW_SKILLS_PATH}}/tts-say/
```
...
# 2. say 엔진 테스트
tts-say speak --engine say --voice Yuna "say 엔진 테스트"

# 3. sag 엔진 테스트 (API 키 있는 경우)
tts-say speak --engine sag "sag 엔진 테스트"

# 4. auto 모드
tts-say "자동 엔진 선택 테스트"
...
| 증상 | 원인 | 해결 |
|------|------|------|
| `tts-say: command not found` | PATH 미등록 | `echo $PATH` 확인, `~/.local/bin` 추가 |
...
| `voice "X" not found` | 잘못된 보이스명 | `tts-say voices --engine sag` 로 확인 |
...
| `{{TTS_INSTALL_PATH}}` | tts-say CLI 설치 경로 | `~/.local/bin/tts-say` |
...
# 보이스 변경
tts-say config set sag.voice "다른보이스"
