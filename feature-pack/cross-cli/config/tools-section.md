### 🔀 cross-cli (타 프로바이더 CLI 비대화 위임)

- **스킬**: `/cross-cli` (외부 에이전트 CLI 오케스트레이션)
...
# 페르소나 + 다회 resume 체인
bash ~/.claude/skills/cross-cli/scripts/resume_chain.sh <cli> <DA|designer|architect|-> "R1" ["R2" ...]

# opencode 단독(모델 자동탐색)
bash ~/.claude/skills/cross-cli/scripts/test_opencode.sh [provider/model]
```
