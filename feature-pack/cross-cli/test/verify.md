...
```bash
# 1. SKILL.md 존재 + 이름
test -f ~/.claude/skills/cross-cli/SKILL.md && grep -q 'name: cross-cli' ~/.claude/skills/cross-cli/SKILL.md && echo "✅ SKILL.md" || echo "❌ SKILL.md missing"

# 2. references
test -f ~/.claude/skills/cross-cli/references/per-cli.md && echo "✅ per-cli.md" || echo "❌ per-cli.md"
test -f ~/.claude/skills/cross-cli/references/personas.md && echo "✅ personas.md" || echo "❌ personas.md"

# 3. scripts (실행권한 포함)
for s in selftest test_opencode resume_chain; do
  f=~/.claude/skills/cross-cli/scripts/$s.sh
  test -x "$f" && echo "✅ $s.sh (x)" || echo "❌ $s.sh missing or not executable"
done

# 4. 문법 검사
for s in selftest test_opencode resume_chain; do
  bash -n ~/.claude/skills/cross-cli/scripts/$s.sh && echo "✅ $s.sh syntax" || echo "❌ $s.sh syntax error"
done
