...
```
⏎ attach ^S 메시지 ^H 대기만 ^D 종료뷰 ^U 자동 ^R 갱신 ^X 끝
...
```
⏎ 복구(resume) ^A 좌석뷰 ^R 갱신 ^P/^O 미리보기 ^X 끝
^T 최근순 ^W SNAP우선 ^G 카테고리 · ◌종료 SNAP/CC/CDX/CMD/OC
창:12h(43건) 계보:최신 · ^]+12h ^\-12h ^L 계보
종료>                                                        43/43
...
CLI: `tmm dead [PAT] [--since H] [--all]` · `tmm dp NAME [N]` · `tmm restore NAME [--dry-run]`
