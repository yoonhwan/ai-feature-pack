...
- **`^B` = 창 안 전부 일괄 복구**: 지금 보이는 창(12h)의 종료 세션을 위에서부터 차례로 resume 합니다. 확인 `y` 후 진행, attach 는 하지 않고 끝나면 `✅ n ❌ m / 총` 집계를 보여줍니다. **창을 `^]`로 넓힌 뒤 `^B`를 다시 누르면 새로 드러난 것만 이어서** 복구됩니다(이미 살아있는 것은 목록에서 빠져 있음). `tmuxc restore --scan --loose --select all --go`의 폰판입니다. 좌석당 부팅 대기 최대 60초라 40건이면 10분 남짓입니다.
