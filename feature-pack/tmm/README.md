...
- 연속 빈 줄은 1줄로.
- **줄 끝 패딩을 지웁니다.** tmux 는 pane 폭(216열)까지 공백으로 채워 내보내므로(실측: 줄당 trailing 136자), 그 공백이 폰 60열을 넘겨 «내용 없는 접힘»을 만들고 빈 `↳` 줄로 보였습니다. 이제 줄 끝 공백만 제거하고 끝에 붙은 색 코드는 되돌려 붙입니다.
- **접힘 표시(`↳`)를 없애고 단어 단위로 접습니다.** `--preview-wrap-sign=''` + `wrap-word` — 원본 pane 과 같은 모양으로 이어집니다. 데스크탑 pane 이 216열로 하드 줄바꿈한 문단은 폰에서 한 번 더 접히는데, 이제 단어 중간이 아니라 단어 경계에서 끊깁니다.
