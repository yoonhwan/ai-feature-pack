...
- headroom: `claude-hr.sh` 래퍼가 health 실패 시 직결(fail-open). **글로벌 `ANTHROPIC_BASE_URL` 정적 export 금지**(SPOF, 프록시 죽으면 전 세션 마비).
...
### 케이스 D — cc_cloak encode/decode 게이트

| 방향 | 게이트 |
|------|--------|
| 요청 (tool 이름 CamelCase) | `_use_cc_tool_cloak(base_url)` — 프록시면 ON |
| 응답 (decamelize 복원) | `strip_tool_prefix=_is_anthropic_oauth` |

직결이 아닌데 OAuth 플래그가 꺼지면 **응답만** `McpTerminal` 복원 실패 가능. 의심 시:
```bash
HERMES_CC_TOOL_CLOAK=1   # 강제 ON (게이트웨이 env 또는 LaunchAgent EnvironmentVariables)
```
