...
## 검증 결과
- 민감정보 스캔 `grep -rIE 'yoonhwan|BYZ|xclaw|nanoclaw|vertex|GCP|/Users/'` → **0건**.
...
- **xclaw 크루 적용** — crew 컨테이너 spawn 시 `ANTHROPIC_BASE_URL=http://host.docker.internal:8790` 전파(이미지 무수정). resume cold prefill이 강타깃.
- **rate-limit A/B** — `/api/oauth/usage` 폴러로 압축 on/off의 한도 소모 속도 실측(PoC 2단계).

## 미해결
- **xclaw 세션 모델 확인** — 크루가 작업(bead) 간 컨텍스트를 이어가나(=재prefill 폭식 확정, headroom 강타깃) vs fresh-per-task(이득 제한적). 적용 전 선결 확인 필요.
