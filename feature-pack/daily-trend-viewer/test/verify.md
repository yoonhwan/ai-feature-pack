...
## V1. 로컬 기동
    ...
    sleep 2
    curl -s -o /dev/null -w "%{http_code}" http://localhost:8778/
기대: `200` (본문은 index.html). 실패 시 /tmp/dtv-server.log 확인.

## V2. CSRF 방어
    # (a) Origin 없음 → 403
    curl -s -o /dev/null -w "%{http_code}" -X POST http://127.0.0.1:8778/api/reels/accounts \
    ...
    curl -s -X POST http://127.0.0.1:8778/api/reels/accounts \
      -H "Origin: http://localhost:8778" -H "Content-Type: application/json" \
      -d '{"action":"add","username":"csrf_probe"}'
...
## V3. SSRF 방어 (/api/img)
    ...
    curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:8778/api/img?u=https://evil.example/x.jpg"
    # (b) userinfo 우회 시도 → 400
    curl -s -o /dev/null -w "%{http_code}" -G "http://127.0.0.1:8778/api/img" \
      --data-urlencode "u=https://i.ytimg.com@evil.example/x.jpg"
    # (c) 유사 도메인(접미 아님) → 400
    curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:8778/api/img?u=https://evilytimg.com/x.jpg"
    # (d) http(비 https) → 400
    curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:8778/api/img?u=http://i.ytimg.com/x.jpg"
