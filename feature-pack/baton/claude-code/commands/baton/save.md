...
**활성 루프/골**: <실행 중이던 /loop·ScheduleWakeup·자율모드, 완료기준(SSOT 경로/한 줄) — 새 세션은 프로세스 상태(taskId)를 못 물려받으므로 여기 적어야 재개 가능. 없으면 "없음">
```

- ≤1KB. 추측·기획 금지, 구체적 사실만.
- **활성 루프/골 필드는 생략 금지**: Monitor/ScheduleWakeup/loop는 세션에 종속된 프로세스라 파일 핸드오프로 자동 전달되지 않는다 — 여기 적어두지 않으면 다음 세션에서 조용히 멈춘다.
