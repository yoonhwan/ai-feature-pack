...
BATON_EXECUTION_INSTRUCTION='작업 결과는 사용자에게 자연어로 응답하세요. baton hook이 INTENT/HARNESS를 자동으로 .baton/handoff/.events.jsonl 에 기록합니다 (sidecar).

⚠️ 다음 파일은 직접 편집하지 마세요 (race 위험):
- .baton/handoff/JOURNAL.md
- .baton/handoff/CURRENT.md
- .baton/handoff/NEXT.md

정리는 /baton:save 또는 /baton:finish 호출 시 헤드리스 에이전트가 sidecar(.events.jsonl)를 일괄 정리합니다.

PLAN.md(설계 문서)는 자유 편집 가능. JOURNAL/CURRENT/NEXT는 자동 정리 영역.'
