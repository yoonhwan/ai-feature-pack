...
| alias | 모델 | effort | 역할 |
|-------|------|--------|------|
| `ccs` | Sonnet 4.6 (`claude-sonnet-4-6`) | high | **코딩·작업·라이브러리 서치·작업 하네스** — 구현 루프, 파일 수정, 리서치 보조 |
...
**윈도우 정책**: Sonnet/Haiku는 200K 표준 윈도우만 쓴다. Opus 4.8은 `claude-opus-4-8` 200K와 `claude-opus-4-8[1M]` 1M을 둘 다 쓸 수 있고, `ccd` 기본은 1M이다. Sonnet `[1m]` 요청은 usage credits가 켜진 경우에만 수동으로 지정하며, 이 스택은 Sonnet 1M을 200K로 폴백하지 않는다.

### 표준 3-세션 구조 (2026-06-12 확정 — HIL 지침)

**핵심 원칙: worker(sonnet)의 200K 표준 컨텍스트를 작업 주력으로 태워 토큰 효율을 극대화한다.** orchestrator(opus)는 디스패치·판정·조율만 하며 컨텍스트를 아끼고, 실제 작업량(로그·코드 풀서치·구현)은 sonnet worker가 소화한다.

```
[ccd = orchestrator] opus   — 지휘·테스팅 판정·증거 취합·머지 조율. ctx 절약(디스패치/판정만).
[ccf = designer]     fable  — 문제 정의·수정 기획·설계·막힘 시 문제해결 위임처.
[ccs = worker]       sonnet — 실작업: 로그 풀서치·코드 컨텍스트 풀서치·구현. 200K ctx를 작업 주력으로 사용.
```
...
- **Opus 4.8**: 검증·로그 분석·DA 기본 모델. `claude-opus-4-8` 200K와 `claude-opus-4-8[1M]` 1M 모두 허용(긴 로그·diff 처리).
- **Sonnet 4.6**: 빠르고 비용 효율적. 200K 표준 윈도우로 구현·서치 루프에서 대부분의 실제 작업량 담당.
...
- `zsh -ic 'type ccd'`가 정상일 때만 해석된 명령으로 기동한다.
