...
6. **agent 실행 명령 해석** (아래 "CLI 명령 해석" 섹션 참조):
   - 1순위: `~/.zshrc`의 **역할별 alias**(`ccs`/`ccd`) 체인을 해석한 결과 + `--name {name} --remote-control {name}` 추가
     - **모델 라우팅**(UC1-4): `ccs`(Sonnet 200K 확인·분석 단말 세션) / `ccd`(Opus 1M 오케스트레이트·실구현·기획 세션). 역할→세션 정본은 byz-zion 「세션 모델 라우팅」.
   - alias 해석 실패 시 일반 `claude`로 우회하지 말고 에러로 차단한다.
...
| agent | role | 실행 |
| --- | --- | --- |
| `claude` | `worker/analysis/verify` | `ccs ... --name --remote-control` |
| `claude` | `orchestrator/implementer/planner` | `ccd ... --name --remote-control` |
| `codex` | `worker` | `codex --dangerously-bypass-approvals-and-sandbox -c model_reasoning_effort="medium"` |
| `codex` | `orchestrator/implementer/planner` | `codex --dangerously-bypass-approvals-and-sandbox -c model_reasoning_effort="high"` |
| `omx` | `worker` | `omx --madmax` |
| `omx` | `orchestrator/implementer/planner` | `omx --madmax --high` |
...
## UC1-4: 세션 모델 라우팅 (Sonnet 200K 확인 세션 / Opus 1M 실행 세션)
...
| alias | 모델 | effort | 역할 |
|-------|------|--------|------|
| `ccs` | Sonnet 4.6 (`claude-sonnet-4-6`) | high | **확인·분석 단말 세션** — 로그 분석, BTS/버그 확인, 문서 확인, 아키텍처 문서 확인·업데이트, 코드/라이브러리 서치 |
| `ccd` | Opus 4.8 (`claude-opus-4-8[1m]`) | high | **실행 세션** — 오케스트레이트, 실구현, 기획, 장문 컨텍스트 통합, 최종 의사결정 |
| `cc2` | Opus 4.8 (`claude-opus-4-8`) | high | **일반 Opus 200K 수동 alias** — 기본 tmuxc 라우팅에는 쓰지 않는다. |
| `ccf` | Fable 5 (`claude-fable-5`) | medium | **특수 설계 자문용 수동 alias** — 기본 tmuxc 라우팅에는 쓰지 않는다. |

**윈도우 정책**: Sonnet/Haiku는 200K 표준 윈도우만 쓴다. Opus 4.8은 `claude-opus-4-8` 200K와 `claude-opus-4-8[1m]` 1M을 둘 다 쓸 수 있지만, tmuxc의 구현·기획·오케스트레이트 역할은 명시적으로 `ccd`를 쓴다. Opus 200K가 필요할 때만 `cc2`를 수동으로 지정한다. Sonnet `[1m]` 요청은 표준 라우팅에 넣지 않고, 요청 실패를 200K로 폴백하지 않는다.

### 표준 3-세션 구조 (2026-06-12 확정 — HIL 지침)

**핵심 원칙: Sonnet 200K 확인 세션을 최대한 많이 써서 관찰·검토·정리를 분산한다.** 로그 분석, BTS/버그 확인, 문서 확인, 아키텍처 문서 확인/업데이트, 코드·라이브러리 서치는 `ccs` 단말 세션으로 열고 짧게 리프레시한다. Opus 1M은 오케스트레이트·실구현·기획처럼 긴 문맥 통합과 결정이 필요한 곳에만 태운다.

```
[ccd = orchestrator/planner/implementer] opus 1M    — 지휘·기획·실구현·최종 판정. 긴 문맥 통합.
[ccs  = analysis/check/doc/log]           sonnet 200K — 로그·BTS·문서·아키텍처 확인, 서치, 단말성 분석.
```

Sonnet 확인 세션은 **단말성**으로 사용한다. 한 세션에 오래 누적하지 말고 결과를 Opus 실행 세션에 넘긴 뒤 닫거나 새로 열어 리프레시한다. Opus 실행 세션은 Sonnet 결과를 받아 계획·구현·최종 판단을 유지한다.

**★ 전략참모 (Strategist) — 프로젝트 전략층**: `ccd` 기반 Opus 1M 세션으로 **프로젝트 전반 전략·기획·설계 + codex DA approve loop으로 전략 생산**을 맡긴다. Sonnet 확인 세션이 로그·문서·아키텍처 확인 결과를 공급하고, 전략참모/오케스트레이터가 장문 컨텍스트를 통합해 결정한다.

**경량 변형(2-세션)**: 단순 사이클은 `ccd`(orch/구현/판정) + `ccs`(확인/로그/문서)만으로 돌린다. 별도 자문이 필요할 때만 수동으로 `ccf`를 열고, 결론만 넘긴 뒤 닫는다.

**★ plan↔dev↔orch 의존성 루프 (필수 순환)**: `ccd` 기획/오케스트레이터가 문제정의·기획을 잡고, 구현 세션이 실작업을 수행한 뒤, `ccs` 확인 세션이 라이브 판정·문서·아키텍처 검토를 분담한다. 확인 결과는 다시 `ccd`으로 모아 기획 gap과 리스크를 갱신한다.
...
- **Fable 5**: 특수 자문용. Opus 비용 초과 + 토크나이저 ~30% 토큰↑ = ctx 소비 빠름. 자문 결과가 나오면 즉시 닫아 비용 절감.
- **Fable 윈도우 정책**: ccf는 **1M 윈도우 금지** — 표준 윈도우로 기동, **ctx 80%에서 증류** (일반 70% 예외). ccf alias에 1M suffix가 붙어있으면 제거.
- **Opus 4.8**: 오케스트레이트·실구현·기획 기본 모델. `ccd`는 `claude-opus-4-8[1m]`으로 긴 문맥 통합을 맡고, `cc2`는 Opus 200K 수동 호출용으로만 둔다.
- **Sonnet 4.6**: 빠르고 비용 효율적. 200K 표준 윈도우로 로그 분석, BTS/버그 확인, 문서 확인, 아키텍처 문서 확인·업데이트, 코드·라이브러리 서치에 최대한 많이 쓴다.
- **Haiku**: 200K 표준 윈도우만 사용.
- **Sonnet 1M**: usage credits가 켜진 경우에만 수동 지정. 표준 alias에는 넣지 않고, 요청 실패를 200K로 폴백하지 않는다.
- **`CLAUDE_CODE_DISABLE_1M_CONTEXT`**: 진단용 env. alias나 표준 기동 예시에 넣지 않는다.
- **alias 해석**: `zsh -ic 'type ccs|ccd|cc2|ccf'`로 해석, eval 금지·type 출력만 신뢰.
...
|-----------|--------|-------|------|--------|-------|-----------|
| `#0 ~ #99` | 2자리 | `ccs` | Sonnet 4.6 (`claude-sonnet-4-6`) | high | on | `FB_OPS#0`, `FB_OPS#99` |
| `#100 ~ #999` | 3자리 | `ccd` | Opus 4.8 (`claude-opus-4-8[1m]`) | high | on | `FB_OPS#100`, `FB_OPS#999` |
| `#1000+` | 4자리+ | `ccd` | Opus 4.8 (`claude-opus-4-8[1m]`) | high | on | `FB_OPS#9999`, `FB_OPS#10000` |

**기동 예시 (세션 생성 시 번호별 alias 선택)**:
```bash
# 2자리 — sonnet 200K
ccs --effort high --name "FB_OPS#99" --remote-control "FB_OPS#99"

# 3자리 — opus4.8 1M + high
ccd --name "FB_OPS#999" --remote-control "FB_OPS#999"

# 4자리+ — opus4.8 1M + high
ccd --name "FB_OPS#9999" --remote-control "FB_OPS#9999"
```

**규칙**:
- **세션은 실제 작업이 있을 때만 열기** — 번호 tier가 정해졌다고 미리 오픈하지 말 것. 태스크 주입 직전에 생성하고, 작업 완료 후 닫는다. 작업 없는 대기 세션은 비용·ctx 낭비.
- alias가 정의되지 않은 경우(`ccs` 미정의): 일반 Claude나 다른 모델로 우회하지 말고 에러로 차단한 뒤 alias를 먼저 수정한다.
- ccf는 특수 자문용 수동 alias로만 쓴다. 번호 규약의 기본 라우팅에는 포함하지 않는다.
...
워크트리 경로 → 절대 경로 변환, 그 다음 UC1과 동일하게 `git rev-parse --show-toplevel`로 root 재확인 (워크트리 내부 sub-dir이 들어와도 root로 정규화). 세션명 = root basename. 나머지(claude 실행 명령은 역할에 따라 ccs|ccd|cc2|ccf alias 해석, UC1-4 기준)는 UC1과 동일.
...
**증류 시 모델 선택 (`ccs` 확인 세션 ↔ `ccd` 실행 세션)**
...
- **업그레이드 흐름**: ccs(확인/분석) → ccd(기획/구현/오케스트레이트) — 결정·장문 통합이 필요할 때
...
**예시 (실측 alias, 2026-06)**:
```
$ zsh -ic 'type ccs; type ccd'
ccs  is an alias for ~/.headroom/claude-hr.sh --dangerously-skip-permissions --model claude-sonnet-4-6 --effort high
ccd is an alias for cc
cc is an alias for ~/.headroom/claude-hr.sh --dangerously-skip-permissions --model "claude-opus-4-8[1m]" --effort high
```
→ 최종 send-keys 명령:
```
~/.headroom/claude-hr.sh --dangerously-skip-permissions --model "claude-opus-4-8[1m]" --effort high --name {name} --remote-control {name}
```
- `ccd`는 dangerously-skip-permissions + opus-4.8-1M로 기동(권한 프롬프트 없이 자율 작업).

**alias 해석 실패 시**:
- 일반 `claude`로 우회하지 말고 에러로 차단한다.
- 역할에 필요한 alias(`ccs` 또는 `ccd`)가 정상일 때만 해석된 명령으로 기동한다.
...
- ccs/ccd/cc2 alias 해석을 위해 `~/.zshrc`가 interactive zsh에서 source 되어야 함 (대부분 기본값)
