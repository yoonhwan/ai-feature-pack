...
- ★**`#<seq>` 는 그 발주의 큐 번호다 (2026-09-06 개정, F5)**★ — 큐가 발급하는 단조증가 번호라 **발주마다 고유하다.** 이전엔 전부 `[발주 도착]` 이라 **연속 두 발주가 구분되지 않았다** — 사람도 「방금 뜬 게 몇 번째인지」를 몰랐고, 판정기(`ft-send-verified.sh`)도 «이번» 발주만 골라낼 수 없었다. 번호가 붙으면 둘 다 해결된다. **재발주하면 새 번호가 나온다** — 1차 번호로 재판정하면 그 번호는 두 번 다시 안 나오므로 영원히 미도달로 보인다.
