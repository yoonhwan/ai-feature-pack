...
```
[발주 #<seq>] bash <mbox 절대경로> recv <세션명> — 실행하고 그 안의 지시를 즉시 착수하라
```
...
- **절대경로로 주입한다** — 상대경로면 수신자 cwd 에서 어긋난다. 가변부는 allowlist 통과 세션명 + 숫자뿐이라 명령 삽입은 막혀 있다.
- ★**`#<seq>` 는 그 발주의 큐 번호다 (2026-09-06 개정, F5)**★ — 큐가 발급하는 단조증가 번호라 **발주마다 고유하다.** 이전엔 전부 `[발주 도착]` 이라 **연속 두 발주가 구분되지 않았다** — 사람도 「방금 뜬 게 몇 번째인지」를 몰랐고, 판정기(`ft-send-verified.sh`)도 «이번» 발주만 골라낼 수 없었다. 번호가 붙으면 둘 다 해결된다. ★**재발주해도 번호는 그대로다 (2026-09-06 개정, 나)**★ — 재발주는 `{mbox} ring <좌석> <최초 seq>` 로 **큐를 건드리지 않고 doorbell 만** 다시 울린다. 미도달의 실체는 「본문이 큐에 없다」가 아니라 「창에 뜬 doorbell 을 좌석이 삼켰다」이다 — 본문은 1차에 이미 큐에 온전하다. 다시 넣으면 좌석이 recv 에서 같은 발주를 2건 본다(실측: `pending=2`, `mailbox.jsonl` 2행). ★이전 판(F5)에는 「재발주하면 새 번호가 나온다」고 적혔다 — 그 문장은 이 개정으로 거짓이 됐다.★
...
### ★도달 판정은 «jsonl 층»에서 한다 — pane capture 로 판정하지 마라 [HARD, 2026-09-06]★

**화면은 폭에 따라 임의 위치에서 줄바꿈되어 어떤 문자열이든 쪼갠다.** 실측(2026-09-06): 32컬럼 pane 에서 `[발주 …]` 프리픽스가 줄 경계에 걸려 쪼개졌고, `capture-pane | grep` 이 그것을 **미도달로 오판**해 **같은 발주가 2회 투입**됐다. 기록에는 줄바꿈이 안 섞인다 — 실측한 jsonl `type=="user"` 레코드는 653자 본문을 **통째로 한 줄**에 담고 있었고 줄바꿈은 **0개**였다.

- **판정 도구는 `ft-reach-check.sh`(세션 jsonl `type=="user"` 레코드 증분)다.** `capture-pane` 은 미도달·도달·재렌더 셋을 전부 같은 값(DIRTY)으로 찍어 **못 가른다**(§2 Step 4 각주와 같은 근거).
- ⇒ **마커가 «짧아야 한다»는 요구는 판정 층에 없다.** 마커를 짧게 두는 이유는 **사람이 화면에서 읽을 때** 편하기 때문이지 판정 때문이 아니다. 이 구분을 뒤집어 적으면 다음 사람이 판정 층을 다시 pane 으로 되돌린다.
---
