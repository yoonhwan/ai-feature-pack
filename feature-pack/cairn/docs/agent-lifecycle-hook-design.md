...
5. **recovery 필드(`execution_ref`/`branch`/`spawned_from`/`session_ref`)가 항상 비어 있다** — status 미갱신과 동일 부류의 버그(지침·룰·스크립트 부족). ① baton `wt-create`가 워크트리를 만들어도 `cairn link --execution-ref`를 호출하지 않고 ② cairn git 훅(post-checkout/post-merge → `cairn reconcile`)이 repo에 설치조차 안 되며(현 `.git/hooks/post-checkout`은 git-lfs 것) ③ 워크트리·브랜치 생성 시 `cairn link`를 부르라는 규칙이 없다. 결과: `cairn map`(recovery-graph)이 항상 빈 워크트리/브랜치 그래프를 그린다. (2026-07-02 v6-realtime-live 원장에서 실증 — 전체가 단일 워크트리 1개로 뭉쳐 fan-out 시각화 불가.)
...
**5. recovery 필드 자동 기록 (신규 — 문제 5 대응)**
status 자동화와 같은 훅·같은 순간을 재사용한다. `session-start` 분기는 이미 branch를 읽어 task를 특정하므로, task 특정에 성공하면 그 task에:
- `execution_ref` = 현재 워크트리 경로(`git rev-parse --show-toplevel`), `branch` = 현재 브랜치명을 기록(비어 있거나 값이 다를 때만 갱신, 멱등).
- 기록 시점은 todo→doing 전환과 동일 트랜잭션에 묶어 커밋 1회로 처리.
보강 wiring(단발성, install.sh가 담당):
- cairn git 훅(post-checkout/post-merge → `cairn reconcile`)을 repo `.git/hooks/`에 **실제 설치**(현재 미설치 — 문제 5의 ②). core.hooksPath/기존 git-lfs 훅과 공존하도록 append 또는 chain.
- baton `wt-create` 커맨드 문서에 "생성 직후 `cairn link <node> --execution-ref <path> --branch <branch>` 호출" 절차 추가(문제 5의 ①·③ — 지침/스크립트 부족 해소). `spawned_from`/`return_to`는 기존 `cairn spawn`/`return` 경로가 이미 담당하므로 신규 없음.
- 결과: `cairn map`이 실제 워크트리/브랜치 fan-out을 그린다(단일 워크트리로 뭉치던 현상 해소).

**6. installer 멱등 wiring + 업데이트 가이드 (신규)**
...
- 위 5의 git 훅 설치도 install.sh가 멱등 수행(이미 있으면 스킵).
...
- `session-start` 분기: 테스트 워크트리에서 `CAIRN_HOOK_EVENT=session-start` 직접 실행 → todo→doing 확인 + `execution_ref`/`branch`가 해당 task에 기록됐는지 확인. 모호 케이스(브랜치 무관/복수 doing) → no-op 확인. 재실행 시 값 동일 → 커밋 no-op(멱등) 확인.
- git 훅 설치: install.sh 후 `.git/hooks/post-checkout`에 cairn reconcile이 들어가고 기존 git-lfs 훅과 공존(둘 다 실행)하는지 확인.
...
| recovery 필드(execution_ref/branch) 빈 값도 버그인가 | 그렇다 — status 미갱신과 동일 부류(지침·룰·스크립트 부족). session-start 훅에서 자동 기록 + wt-create/git-hook wiring으로 대응(컴포넌트 5). 2026-07-02 plan-view 실험 중 발견. |
